...
def setup_tmux(
    ...
    watchdog_exe="/home/palpidefs/TB_SPS_June_2023/opamp-utils/watchpuppy.py"
    pusher_exe="/home/palpidefs/TB_SPS_June_2023/opamp-utils/pusher.py"
    ...
    watch=session.new_window("watchdog")
    watch.split_window()
    watch.select_layout("tiled")
    server.cmd('select-pane','-t','ITS3:watchdog.0','-T','Watch puppy')
    server.cmd('select-pane','-t','ITS3:watchdog.1','-T','Pussher')
    ...
    watch.select_pane(f"ITS3:watchdog.0").send_keys(f'{watchdog_exe} puppyshouse')
    watch.select_pane(f"ITS3:watchdog.1").send_keys(f'{pusher_exe} testbot')

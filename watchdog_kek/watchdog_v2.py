...
if __name__ == "__main__":
    ...
    parser.add_argument("--interval", "-i", default=5, type=float, help="Monitoring interval in min (default 5).")
        ...
        with open("log_watchdog.log","a") as file: 
            try:
                ...
                if dt % interval == 0 or isFirst == True:
                    isFirst = False
                    beamrate1 = isBeamTrigger(triggerdt,"XXRX")
                    beamrate2 = isBeamTrigger(triggerdt,"XXXR")
                    ...
                    if 0 <= nowhour < 8:
                        ...
                        next_shifter = json_shift[str(now.day)]["Morning"]
                    elif 8 <= nowhour < 16:
                        shifter = json_shift[str(now.day)]["Morning"]
                        next_shifter = json_shift[str(now.day)]["Afternoon"]
                    elif 16 <= nowhour < 24:
                        shifter = json_shift[str(now.day)]["Afternoon"]
                        next_shifter = json_shift[str(now.day+1)]["Night"]
                    else :
                        shifter = "No Shifter"
                        next_shifter = "No Shifter"
...
"EUDAQ" status means if EUDAQ operation window is opening. It couldn't mean EUDAQ is fine.                
                    """.format(now,beamrate1/triggerdt,beamrate2/triggerdt,EUDAQ_status,shifter,next_shifter)
